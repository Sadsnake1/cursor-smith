...
import type { SettingDefinitionItem, SettingDefinitionGroup, SettingDefinitionRender, SettingGroupItem, SliderComponent } from "obsidian";
...
import type { DropdownOptions, Look, LookCards, LookSettingsHooks, PresetRowActions, RowOptions, SettingKey, SliderOptions, SwatchOptions } from "./types";
...
export class CursorSmithSettingTab extends PluginSettingTab {
  ...
  vimCursorsGroup(): SettingDefinitionGroup {
    ...
    // Shown only while Obsidian's own Vim key bindings are off - two
    // wordings, one per state of the toggle above, so that a flip of it
    // is a visibility refresh and not a re-render.
  ...
  lookDefinitions({ get, set, renderCursorStyleSetting, renderTorchToggleSetting }: LookSettingsHooks): LookCards {
    ...
    // refreshDomState() - a class toggle per row, no re-render. That is what
    // a gate's write calls. update(), which hands Obsidian a new tree and
    // has it rebuilt and reconciled, costs 60-80ms for this panel's 70 rows
    // (measured in the 1.13.7 settings window) and is kept for the one
    ...
    // --- Row builders ---------------------------------------------------------
    const row = (name: string, desc: string, build: (s: Setting) => void, { depth = 0, when }: SwatchOptions = {}) => {
      const def = this.row(name, desc, build, depth);
      if (when) def.visible = when;
      return def;
    };
    const toggle = (name: string, desc: string, key: keyof Look, { depth = 0, gate = false, when }: RowOptions = {}) =>
      row(name, desc, (s) => { s.addToggle((t) => t.setValue(!!get(key)).onChange(gate ? redraw(key) : set(key))); }, { depth, when });
    ...
    const slider = (name: string, desc: string, key: keyof Look, [min, max, step]: number[], { depth = 0, fallback, gate = false, when }: SliderOptions = {}) =>
      row(name, desc, (s) => {
    ...
    const swatchRow = (name: string, keys: (keyof Look)[], desc: string, { depth = 0, when }: SwatchOptions = {}) =>
      row(name, desc, (s) => {
    ...
    appearance.push(toggle("Show letter inside cursor", "Shows the letter inside the block, colors flipped. Does nothing while Hollow or Translucent is on.",
    ...
    appearance.push(swatchRow("Colors (dark theme)", keys("gradientDark"),
      "In order from the top of the cursor to the bottom — or left to right for the Underline style.", { depth: 1, when: gradient }));
    ...
    // One row, two swatches - dark theme then light - the pattern the
    // gradient rows use for "several pickers that belong to one idea".
    appearance.push(swatchRow("Cursor color", ["colorDark", "colorLight"],
      "Dark theme first, then light. Neon colors that look right on a dark background wash out on a white page.", { when: off("gradientEnabled") }));
    ...
    // Rows here that read gates living in other cards - Gradient decides
    // whether Pixel trail's Gradient colors, Energy beam's Aurora and Neon's
    // Gradient trail are offered, Blinking whether the torch's Sync with
    // blink is - just say so in their predicate.
    const effects = [];
    ...
    effects.push(toggle("Backspace disintegration", "Deleting throws a burst outward in inverted colors; with Rainbow on, the next color in the sweep.",
    ...
    effects.push(slider("Bolt strength", "How brightly the strike shows; with Rainbow on, each bolt takes a color.", "thunderstrikeStrength", [0.1, 1, 0.05],
    ...
    // a Rainbow toggle is a switch that visibly does nothing. Same rule as
    // Sync with blink under the torch, and Gradient colors under Pixel trail.
    ...
    effects.push(slider("Gravity direction", "Which way the pull goes, in degrees. 0 is down, 90 right, 180 up, 270 left.", "flameTrailGravityAngle", [0, 359, 5],
    ...
    effects.push(slider("Idle delay", "How long (in ms) the cursor must sit still before the stardust starts.", "stardustDelayMs", [500, 8000, 250],
    ...
    effects.push(swatchRow("Stages (dark theme)", heat("speedHeatDark"),
      "Warming to flat out, left to right. At rest the cursor keeps its own color.", { depth: 2, when: stages }));
    ...
    // gate in drawHotHead. Also hidden without Speed demon itself, since
    // there would be no heat to follow.
    ...
    effects.push(slider("Pulse depth", "How far the light closes at its darkest, as a share of Light size. At 1 it goes out.", "overlayBlinkDepth", [0.05, 1, 0.05],
    ...
    effects.push(slider("Flicker depth", "How far the flame swings either side of Glow strength. At 1 it gutters right out.", "overlayFlickerAmount", [0.05, 1, 0.05],
